...
REM Zig C compiler wrapper for Linux x86_64 musl
REM Some build scripts pass Rust-style triples like --target=x86_64-unknown-linux-musl.
...
REM cc-rs probes MSVC via `-?` / `/?`. Zig accepts `-?` oddly; force failure so cc-rs doesn't treat us as cl.exe.
if /I "!arg!"=="-?" exit /b 1
if /I "!arg!"=="/?" exit /b 1
...
if /I "!arg!"=="--target=x86_64-unknown-linux-musl" (
	shift
	goto loop
)
if /I "!arg!"=="--target" (
	REM If next is a Rust-style triple, drop both
	if /I "%~2"=="x86_64-unknown-linux-musl" (
		shift
		shift
		goto loop
	)
